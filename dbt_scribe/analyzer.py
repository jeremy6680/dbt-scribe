...
AGGREGATION_PATTERN = re.compile(r"\b(count|sum|avg|min|max|median)\s*\(", re.IGNORECASE)
...
def _extract_select_expressions(compiled_code: str, adapter_type: str | None) -> dict[str, str]:
    try:
...
def _extract_select_expressions_fallback(compiled_code: str) -> dict[str, str]:
    ...
    for column in re.findall(r"(?im)^\s*([a-zA-Z_][a-zA-Z0-9_\.]*)\s*,?$", compiled_code):
